...
var flag_fn = '' // Флаги для отображения столбцов
var flag_ln = ''
var flag_ph = 'hidden' //телефон изначально показан не будет
var flag_ab = ''
var flag_ec = ''
...
let render = (data) => { //Функция отображения таблицы
    ...
    if (document.getElementById('f_name').checked == true) {
        flag_fn = ''
    } else {
        flag_fn = 'hidden'
    }
    if (document.getElementById('l_name').checked == true) {
        flag_ln = ''
    } else {
        flag_ln = 'hidden'
    }
    ...
    //Считаем индексы начала и конца элементов отображаемых на странице
    let len = data.length
    ...
    //динамически создаем эллементы таблицы
    ...
    //создаем панель навигации по странице
...
//функция получения данных о пользователе в форму для изменения этих данных
let Update = (e) => {
    id = e.id
...
// функция отображения и скрытия столбцов таблицы
let check = (e) => {
